...
VirtManager.prototype.actions = function (req, res) {
  console.log("VirtController.actions");
  ...
  var options = {
    ip: ip,
    route: route + "/" + name,
    sesCookie: req.cookies['connect.sid']
  };
...
VirtManager.prototype.dashboardView = function (req, res) {
  console.log("VirtManager.dashboardView");
  console.log("req.session: ", req.session);
...
VirtManager.prototype.hostModels = function(req, res) {
  ...
  var sesCookie = req.cookies['connect.sid']
  host.findAll(sesCookie, function (models) {
    res.json(models);
  })
};
